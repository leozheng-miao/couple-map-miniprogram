...
当前版本为 `1.1.0`，已完成微信审核前的功能补齐与界面细节打磨。

## 核心能力

- 双人情侣空间：支持创建空间、邀请码加入空间、刷新邀请码；空间满员后不再展示邀请码操作。
- 个人资料：支持在“我的”页面更新用户昵称、头像和空间名称。
- 地图回忆记录：在地图上展示两个人共同记录过的地点。
- 地点管理：支持新增、编辑、删除餐厅、酒店、景点、娱乐、特殊和其他类型地点。
- POI 搜索：通过腾讯位置服务 WebService API 搜索地点。
- 手动选点：无搜索结果或未配置地图 Key 时，可在地图上手动选择坐标。
- 照片记录：每个地点可上传多张照片，支持删除照片和设置主图；只剩一张照片时禁止删除。
...
`1.1.0` 聚焦完整可用的私密双人地图体验，并补齐个人资料、空间资料、照片主图、照片删除、空间统计和关键按钮对齐细节。不包含复杂社交关系、公开内容流、商业化、后台管理系统或多空间切换。所有核心读写都通过云函数完成，数据库集合不开放客户端直接写入。
...
- `note-form`：新增和编辑便利贴。
- `album`：空间相册。
- `profile`：个人资料、空间资料、邀请码、统计、导出入口和设置入口。
...
| `createSpace` | 创建情侣空间 |
| `joinSpace` | 通过邀请码加入空间 |
| `getCurrentSpace` | 获取当前用户所在空间和空间统计 |
| `updateProfile` | 更新用户昵称、头像和空间名称 |
...
6. 逐个部署 `cloudfunctions/` 下的业务云函数，选择“创建并部署：云端安装依赖”。1.1.0 版本必须包含 `updateProfile` 和新版 `getCurrentSpace`。
...
- 核心路径：创建空间、加入空间、新增地点、上传照片、删除照片、设置主图、新增便利贴、相册查看。
- 个人资料：昵称、头像、空间名称可保存，切换页面后仍保持最新值。
- 统计数据：“我的”页面地点数和便利贴数与实际空间数据一致。
